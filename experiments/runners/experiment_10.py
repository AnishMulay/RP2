...
from clustered_push_relabel.solvers.simple_bipartite_2 import SimpleGPUSolver2
...
WARMUP_RUNS = 0
TIMED_RUNS = 1
METHODS = ("Exact", "ProxyExact", "DiscreteProxyExact", "Simple2")
...
def normalize_points(P_red, P_blue):
    ...
    diameter is a Python float for passing to SimpleGPUSolver2.
    """
...
def benchmark_simple2(P_red, P_blue, device, epsilon, sample_factor):
    P_red_norm, P_blue_norm, diameter = normalize_points(P_red, P_blue)

    for _ in range(WARMUP_RUNS):
        empty_cache_if_cuda(device)
        solver = SimpleGPUSolver2(
            P_red_norm,
            P_blue_norm,
            epsilon,
    ...
    adj_stats = {
        "samples": int(solver.DR_int.shape[0]),
    ...
    try:
        ...
        if method_name == "Exact":
            ...
            (
                ...
                feasibility,
                adj_stats,
            ) = benchmark_simple2(P_red, P_blue, device, epsilon, sample_factor)
        else:
...
def print_violation_table(rows):
    ...
    for row in rows:
        n = row["n"]
        f = row["results"]["Simple2"].get("feasibility")
        if f is None:
            feas_viol = feas_worst = admis_viol = admis_worst = None
        else:
            feas_viol   = f["feas_violations"]
            feas_worst  = f["feas_worst_excess"]
            admis_viol  = f["phase_violations"]
            admis_worst = f["phase_worst_abs_diff"]
...
def print_timing_table(rows):
    print()
    print(
        "  N    | ExactT(ms) | ProxyT(ms) | DiscProxyT(ms) | Simple2T(ms) | Phases"
    )
    print(
        "-------|------------|------------|----------------|--------------|-------"
    )
    ...
    for row in rows:
        ...
        print(
            ...
            f"{format_time(simple2['time_ms']):>12} | "
            f"{format_phases(simple2['phases']):>6}"
        )
...
def print_cost_table(rows):
    print()
    print(
        "  N    | ExactAvg | ProxyAvg | DiscProxyAvg | Simple2Avg"
    )
    print(
        "-------|----------|----------|--------------|-----------"
    )
    ...
    for row in rows:
        ...
        print(
            ...
            f"{format_avg_cost(simple2['avg_cost']):>9}"
        )
...
def print_error_breakdown_table(rows):
    ...
    for row in rows:
        ...
        simple2_avg = row["results"]["Simple2"]["avg_cost"]

        proxy_err = diff_or_nan(proxy_avg, exact_avg)
        disc_err = diff_or_nan(disc_avg, proxy_avg)
        solver_err = diff_or_nan(simple2_avg, disc_avg)
        total_err = diff_or_nan(simple2_avg, exact_avg)
...
def configure_experiment():
    ...
    """
    print("\n" + "=" * 50)
    print("  Experiment 10 Configuration")
    print("=" * 50)
...
def main():
    ...
    print("Experiment 10: Simple2 vs Exact, Synthetic 2D", flush=True)
